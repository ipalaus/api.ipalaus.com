...
class FeatureContext extends BehatContext
{
    ...
    public function __construct(array $parameters)
    {
        ...
        $this->client->setDefaultHeaders([
            'Accept' => 'application/vnd.com.ipalaus.api-v1+json'
        ]);
    }
    ...
    /**
     * @Given /^I have the token "([^"]*)"$/
     */
    public function iHaveTheToken($accessToken)
    {
    ...
    /**
     * @Given /^the "([^"]*)" property starts with "([^"]*)"$/
     */
    public function thePropertyStartsWith($property, $prefix)
    {
        $payload = $this->getScopePayload();
        ...
        assertStringStartsWith(
            ...
            $payload->$property,
            sprintf(
                "Asserting the [%s] property starts with [%s].\n%s",
                $property,
                $prefix,
                json_encode($payload)
            )
    ...
    /**
     * @Given /^the "([^"]*)" property starts with insensitive "([^"]*)"$/
     */
    public function thePropertyStartsWithInsensitive($property, $prefix)
    {
        $payload = $this->getScopePayload();

        assertStringStartsWith(
            strtolower($prefix),
            strtolower($payload->$property),
            sprintf(
                "Asserting the [%s] property starts with [%s].\n%s",
                $property,
                $prefix,
    ...
    /**
     * @Given /^the "([^"]*)" header exists$/
     */
    public function theHeaderExists($key)
    {
        $headers = $this->response->getHeaders();

        assertTrue(
            $headers->offsetExists($key),
            "Asserting the [$key] header exists.\n".json_encode($headers)
        );
    }
    ...
    /**
     * @Given /^scope into the parent "([^"]*)" property$/
     */
    public function scopeIntoTheParentProperty($scope)
    {
